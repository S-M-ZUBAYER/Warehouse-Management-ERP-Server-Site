...
router.post('/', requireRole('owner'), createRoleValidator, rolesController.createRole);

// PUT  /api/v1/roles/:id      — update role (owner/admin only)
router.put('/:id', requireRole('owner'), updateRoleValidator, rolesController.updateRole);

// PATCH /api/v1/roles/:id/permissions  — update only permissions (owner/admin only)
router.patch('/:id/permissions', requireRole('owner'), rolesController.updatePermissions);

// DELETE /api/v1/roles/:id   — delete role (owner/admin only)
router.delete('/:id', requireRole('owner'), rolesController.deleteRole);
