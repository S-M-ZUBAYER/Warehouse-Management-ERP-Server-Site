...
// Default permission structure matching the real sidebar pages.
    ...
    inventory_management: {
        ...
        sub: {
            ...
            sku_mapping: {
                access: false,
                sub: {
                    sku_mapping_by_product: false,
                    sku_mapping_by_merchant: false,
                },
            },
            inventory_list: false,
            manual_inbound: false,
            inbound: {
                access: false,
                sub: {
                    inbound_draft: false,
...
const buildPermissions = (supplied = {}) => {
    ...
    const mergeRecursive = (target, source) => {
        ...
        // Backward compatibility: if a permission node is sent as a boolean,
        // apply it to access for object nodes, or to the boolean leaf itself.
        if (typeof source === 'boolean') {
            if (target && typeof target === 'object' && Object.prototype.hasOwnProperty.call(target, 'access')) {
                target.access = source;
            }
            return;
        }
        ...
        if (source.sub && target.sub) {
            for (const [key, value] of Object.entries(source.sub)) {
                if (!Object.prototype.hasOwnProperty.call(target.sub, key)) continue;
                if (typeof value === 'object' && value !== null && typeof target.sub[key] === 'object') {
                    mergeRecursive(target.sub[key], value);
                } else if (typeof target.sub[key] === 'object' && target.sub[key] !== null) {
                    mergeRecursive(target.sub[key], value);
                } else {
...
const getRoles = async (user, filters = {}) => {
    ...
    // Do not serve role list from cache. The UI needs the latest linked
    // sub-account count and updated_at immediately after role/sub-account edits.
...
const updateRole = async (user, roleId, data) => {
    ...
    // Replace the saved permission object with the form's current permission state.
    // Merging with the old JSON keeps unchecked old permissions alive, so use the
    // submitted payload as the source of truth.
    if (data.permissions !== undefined) {
        updates.permissions = buildPermissions(data.permissions);
    }
