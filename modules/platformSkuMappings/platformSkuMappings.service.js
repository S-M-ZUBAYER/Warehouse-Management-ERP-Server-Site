...
const getPlatformSkuMappings = async (user, filters = {}) => {
    ...
    if (Array.isArray(permittedStoreIds)) {
        if (!permittedStoreIds.length) return { data: [], pagination: { total: 0, page: parseInt(page), limit: parseInt(limit), totalPages: 0 } };
        if (platformStoreId && !permittedStoreIds.includes(Number(platformStoreId))) {
            ...
            where.platform_store_id = { [Op.in]: permittedStoreIds };
        }
    }
...
const createPlatformSkuMapping = async (user, data) => {
    ...
    if (merchantSkuId) {
        const sku = await MerchantSku.findOne({ where: { id: merchantSkuId, company_id: user.companyId, deleted_at: null } });
        if (!sku) { const err = new Error('Merchant SKU not found'); err.statusCode = 400; throw err; }
    }
    if (combineSkuId) {
        const sku = await CombineSku.findOne({ where: { id: combineSkuId, company_id: user.companyId, deleted_at: null } });
    ...
    const mapping = await PlatformSkuMapping.create({
        ...
        platform_model_id:        platformModelId      || null,
        platform_open_id:         platformOpenId      || null,
        platform_cipher_id:       platformCipherId    || null,
        platform_product_id:      platformProductId   || null,
        platform_warehouse_id:    platformWarehouseId || null,
        platform_item_id:         platformItemId      || null,
        platform_location_id:     platformLocationId  || null,
        // ────────────────────────────────────────────────────────────────────
