...
const { saveAvatarFile } = require('../../utils/avatar');
...
const login = async ({ email, password }) => {
    ...
    return {
        ...
        user: {
            ...
            role: user.role,
            roleId: user.role_id,
            roleName: user.roleInfo?.name || user.role,
            isOwner: String(user.role || '').toLowerCase() === 'owner',
            permissions: parsePermissions(user.roleInfo?.permissions),
...
const updatePassword = async ({ email, newPassword }) => {
    ...
    await redis.del(sessionKey(user.company_id, user.id));
...
const createSubAccount = async (adminUser, data) => {
    ...
    if (String(adminUser.role || '').toLowerCase() !== 'owner') {
    ...
    await redis.flushByPattern(`company:${adminUser.companyId}:cache:roles*`);
...
const updateSubAccount = async (adminUser, userId, data) => {
    const { User, Role, UserStorePermission, UserWarehousePermission } = require('../../models');
    ...
    await sequelize.transaction(async (t) => {
        ...
        if (data.roleId !== undefined) {
            const role = await Role.findOne({ where: { id: data.roleId, company_id: adminUser.companyId }, transaction: t });
            if (!role) {
                const err = new Error('Invalid role for this company');
                err.statusCode = 400;
                throw err;
        ...
        if (data.avatar !== undefined) updates.avatar_url = await saveAvatarFile(data.avatar);
...
const upsertSubAccount = async (adminUser, data) => {
    ...
    if (existingElsewhere) {
        ...
        await existingElsewhere.update({
            ...
            avatar_url: data.avatar ? await saveAvatarFile(data.avatar) : existingElsewhere.avatar_url,
    ...
    await redis.flushByPattern(`company:${adminUser.companyId}:cache:roles*`);
        ...
        await redis.flushByPattern(`company:${existingElsewhere.company_id}:cache:roles*`);
