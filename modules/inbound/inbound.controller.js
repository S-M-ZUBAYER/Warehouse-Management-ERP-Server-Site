...
const createManualInbound = async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return sendError(res, 'Validation failed', 400, errors.array().map(e => ({ field: e.path, message: e.msg })));
        }
        const result = await service.createManualInbound(req.user, req.body);
        return sendSuccess(res, 'Manual inbound receipt created successfully', result, 201);
    } catch (err) { next(err); }
};

// GET /api/v1/inbound/manual  — list manual inbound receipts only
const getManualInboundOrders = async (req, res, next) => {
    try {
        const result = await service.getManualInboundOrders(req.user, req.query);
        return sendSuccess(res, 'Manual inbound orders fetched successfully', result.data, 200, result.pagination);
    } catch (err) { next(err); }
};
