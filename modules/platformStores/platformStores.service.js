...
// ─── Update ────────────────────────────────────────────────────────────────────
// Public create without ERP JWT. Caller must provide companyId.
const createPublicPlatformStore = async (data) => {
    const { Company, PlatformStore, Warehouse } = require('../../models');
    ...
    const existing = await PlatformStore.findOne({
        where: { company_id: companyId, platform, external_store_id: externalStoreId },
    });
    if (existing) {
        const err = new Error(`A ${platform} store with this external store ID is already connected`);
        err.statusCode = 409;
    ...
    if (defaultWarehouseId) {
        const wh = await Warehouse.findOne({ where: { id: defaultWarehouseId, company_id: companyId } });
        if (!wh) {
            const err = new Error('Invalid default warehouse');
            err.statusCode = 400;
            throw err;
    ...
    return PlatformStore.findOne({
        where: { id: store.id, company_id: companyId, deleted_at: null },
        attributes: { exclude: ['access_token', 'refresh_token', 'webhook_secret'] },
        include: [{
            model: Warehouse,
            as: 'defaultWarehouse',
