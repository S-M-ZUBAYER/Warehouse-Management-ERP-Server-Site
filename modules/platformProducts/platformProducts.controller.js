...
// GET /api/v1/platform-products/:productId/merchant-sku-options
// Options for product-side mapping/change modal.
const getMerchantSkuOptions = async (req, res, next) => {
    try {
        const result = await service.getMerchantSkuOptionsForProduct(req.user, req.params.productId, req.query);
        return sendSuccess(res, result.message || 'Merchant SKU options fetched', result);
    } catch (err) { next(err); }
};
