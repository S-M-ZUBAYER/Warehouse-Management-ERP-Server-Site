...
/**
 * syncShopeeStore
 * Calls Java proxy: GET /new-shopee-open-shop/api/dev/product/get_item_list/:shopId
 * Paginates using next_offset until has_next_page = false.
 * For each item, calls GET /new-shopee-open-shop/api/dev/product/get_item/:shopId?itemId=
...
const syncShopeeStore = async (store, PlatformProduct) => {
    ...
    while (hasNextPage) {
        const listRes = await javaApi.post(
            `/new-shopee-open-shop/api/dev/product/get`,
            {
                offset,
                page_size:   pageSize,
                item_status: ['NORMAL'],
...
/**
 * syncTikTokStore
 * Calls Java proxy: POST /tiktokshop-partner-country/api/dev/products/get/pagination
...
console.log("start",store, PlatformProduct,"Call tiktok store product ....................................1.............................");
    ...
    const fetchTikTokProductDetail = async (productId, fallbackProduct) => {
        try {
            const detailRes = await javaApi.get(
                `/tiktokshop-partner-country/api/dev/products/get`,
    ...
    do {
        ...
        const res = await javaApi.post(
            `/tiktokshop-partner-country/api/dev/products/get/pagination`,
            { status: "ACTIVATE" },
            { params }
        );
...
const updatePlatformStock = async (user, body) => {
    ...
    if (platform === 'shopee') {
        // POST /new-shopee-open-shop/api/dev/product/update_stock/:shopId
        const shopId = mapping.platformStore?.store_shop_id;
        result = await javaApi.post(
            `/new-shopee-open-shop/api/dev/product/update_stock/${shopId}`,
            {
        ...
        // POST /tiktokshop-partner-country/api/dev/products/updateStock
        const openId  = mapping.platformStore?.store_open_id;
        const cipher  = mapping.platformStore?.store_cipher;
        result = await javaApi.post(
            `/tiktokshop-partner-country/api/dev/products/updateStock`,
            {
                skus: [{
