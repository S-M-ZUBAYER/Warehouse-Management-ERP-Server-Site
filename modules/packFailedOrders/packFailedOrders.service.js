...
const listPackFailedOrders = async (user, query) => {
    const { PackFailedOrder } = require('../../models');
...
const upsertPackFailedOrders = async (user, body) => {
    const { PackFailedOrder } = require('../../models');
...
const deletePackFailedOrders = async (user, body) => {
    const { PackFailedOrder } = require('../../models');
    const companyId = resolveCompanyId(user, body.companyId);
    const platform = normalizeString(body.platform);
    const storeId = normalizeString(body.storeId);
    ...
    const deleted = await PackFailedOrder.destroy({
        where: {
            company_id: companyId,
            platform,
            store_id: storeId,
