...
const packStockValidator = [
    ...
    body('order.items.*.quantity').optional().isInt({ min: 1 }),
...
const deleteSkuOverrideValidator = [
    body('platform').notEmpty().withMessage('platform is required').isIn(['shopee', 'tiktok']),
    body('shopId').optional({ nullable: true }).trim().isLength({ max: 100 }),
    body('openId').optional({ nullable: true }).trim().isLength({ max: 100 }),
    body('cipherId').optional({ nullable: true }).trim().isLength({ max: 255 }),
    body().custom((value) => {
        if (!value.platformOrderId && !value.orderId && !value.orderNo && !value.order?.orderId && !value.order?.orderNo && !value.order?.id) {
            throw new Error('orderId is required');
        }
        return true;
    }),
