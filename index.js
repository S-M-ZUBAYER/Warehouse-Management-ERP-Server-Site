...
app.post(
    '/api/v1/platform-stores/public',
    platformStoresRoutes.publicCreateValidator,
    platformStoresController.createPublicPlatformStore
);
app.get(
...
app.use((err, _req, res, _next) => {
    ...
    if (err.type === 'entity.too.large') {
        return res.status(413).json({
            success: false,
            message: 'Request body is too large. Upload an avatar image 5MB or smaller.',
        });
    }

    if (err.name === 'SequelizeDatabaseError' && /data too long/i.test(err.message || '')) {
        return res.status(400).json({
            success: false,
            message: 'One or more fields are too large. Upload an avatar image 5MB or smaller.',
        });
    }
...
const start = async () => {
    ...
    app.listen(PORT, HOST, () => {
        ...
        console.log(`LAN API  -> ${localNetworkUrl}`);
        console.log(`API Docs -> ${appUrl}/api-docs`);
        console.log(`LAN Docs -> ${localNetworkUrl}/api-docs`);
        console.log(`Health   -> ${appUrl}/health`);
        console.log(`LAN Health -> ${localNetworkUrl}/health`);
    });
};
