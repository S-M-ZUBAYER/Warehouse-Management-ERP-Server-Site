...
module.exports = {
    ...
    paths: {
        ...
        '/inbound/manual': {
            get: {
                tags: ['Inbound'],
                summary: 'List manual inbound receipts',
                description: 'Returns only inbound orders created via the manual receipt flow (is_manual: true). All are status=completed.',
                security: [{ bearerAuth: [] }],
                parameters: [
                    { in: 'query', name: 'warehouseId', schema: { type: 'integer' } },
                    { in: 'query', name: 'search', schema: { type: 'string' }, description: 'Search by inbound_id, supplier name or reference' },
                    { in: 'query', name: 'dateFrom', schema: { type: 'string', format: 'date' } },
                    { in: 'query', name: 'dateTo', schema: { type: 'string', format: 'date' } },
                    { in: 'query', name: 'page', schema: { type: 'integer', default: 1 } },
                    { in: 'query', name: 'limit', schema: { type: 'integer', default: 20 } },
                    { in: 'query', name: 'sortBy', schema: { type: 'string', enum: ['created_at', 'updated_at', 'inbound_id'] } },
                    { in: 'query', name: 'sortOrder', schema: { type: 'string', enum: ['ASC', 'DESC'] } },
                ],
                responses: { 200: { description: 'Manual inbound orders fetched' } },
            },
            post: {
                tags: ['Inbound'],
                summary: 'Create manual inbound receipt (direct → completed)',
                description: 'Creates an inbound order that is immediately marked completed and updates qty_on_hand for each line. No draft/ship steps required.',
                security: [{ bearerAuth: [] }],
                requestBody: {
                    required: true,
                    content: {
                            ...
                            schema: {
                                ...
                                properties: {
                                    ...
                                    lines: {
                                        type: 'array', minItems: 1,
                                        items: {
                                            type: 'object', required: ['merchantSkuId', 'qtyReceived'],
                                            properties: {
                                                merchantSkuId: { type: 'integer', example: 1 },
                                                qtyReceived: { type: 'integer', example: 100, description: 'Quantity physically received — added directly to qty_on_hand' },
                                                unitCost: { type: 'number', example: 12.50 },
                                                currency: { type: 'string', example: 'USD' },
                                            },
                                        },
                ...
                responses: {
                    201: { description: 'Manual inbound created and stock updated — status is completed', content: { 'application/json': { schema: { type: 'object', properties: { data: { $ref: '#/components/schemas/InboundOrderResponse' } } } } } },
                    400: { description: 'Validation failed' },
                    403: { description: 'Forbidden' },
                },
            },
