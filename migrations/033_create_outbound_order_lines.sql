-- ============================================================
-- Migration: 012_create_outbound_order_lines.sql
-- Description: Line items within an outbound order — one row per SKU per outbound
-- ============================================================

CREATE TABLE IF NOT EXISTS `outbound_order_lines` (
    `id`                  INT UNSIGNED     NOT NULL AUTO_INCREMENT,
    `company_id`          INT UNSIGNED     NOT NULL,
    `outbound_order_id`    INT UNSIGNED     NOT NULL,
    ...
    -- Per-unit cost at time of outbound (for COGS tracking)
    ...
    -- One SKU per outbound order
    UNIQUE KEY `uq_outbound_order_lines` (`outbound_order_id`, `merchant_sku_id`),

    KEY `idx_iol_company`         (`company_id`),
    KEY `idx_iol_outbound_order`   (`outbound_order_id`),
    KEY `idx_iol_merchant_sku`    (`merchant_sku_id`),
    KEY `idx_iol_discrepancy`     (`has_discrepancy`),

    CONSTRAINT `fk_iol_company`
        FOREIGN KEY (`company_id`)       REFERENCES `companies` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_iol_outbound_order`
  ...
  COMMENT='Line items within an outbound shipment — one row per SKU';